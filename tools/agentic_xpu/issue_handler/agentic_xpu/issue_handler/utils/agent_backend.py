...
class OpenCodeBackend(AgentBackend):
    def run(self, prompt: str, workdir: str | None = None,
        ...
        skills_hint = (
            ...
            f"and {SKILLS_DIR.parents[1] / 'instructions'}. "
        ...
        with open(log_path, "w") as log_f:
            ...
            with open(stderr_path, 'w') as stderr_f:
                ...
                try:
                    ...
                    buf = ""
                    # Idle timeout DISABLED — wall-clock guard above is authoritative.
                    # opencode --format json can be silent for many minutes during
                    # tool calls (notably the cold clangd --background-index pass on
                    # pytorch, which routinely takes 6+ minutes before the first
                    # JSON event). A short idle timeout misfires and kills genuinely
                    # working agents. Set to effective_timeout + 1 so the check
                    # below is structurally preserved but never trips.
                    idle_timeout = effective_timeout + 1
                    ...
                    def _read_lines():
                        ...
                        while True:
                            ...
                            if idle_elapsed > idle_timeout:
                                ...
                                # Report the *idle* threshold that fired, not the
                                # wall-clock budget — otherwise log messages claim
                                # "timed out after 3600s" when the real kill was at
                                # ~5 minutes idle, which is exactly how we wasted
                                # an hour chasing a phantom hang.
                                raise subprocess.TimeoutExpired(
                                    cmd, idle_timeout)
