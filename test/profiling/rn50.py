...
#
# Portions of this file are derived from PyTorchExamples
...
parser.add_argument(
    "--optimize",
    action="store_true",
    help="Use torch.xpu.optimize for training/inference xpu model",
)
parser.add_argument(
    "--kineto_profile",
    action="store_true",
...
parser.add_argument(
    ...
    type=int,
    metavar="N",
    help="mini-batch size for calibration",
)
parser.add_argument(
    "--perchannel-weight",
    default=False,
...
parser.add_argument(
    "--save",
    help="Path to save entile model, save infernce mode, training is not available",
)
...
parser.add_argument("--end-lr", type=float, default=1e-4, help="the end learning rate")
...
def main_worker(ngpus_per_node, args):
    ...
    if args.load:
        ...
        if args.lars and args.xpu is not None:
            ...
            optimizer = torch.xpu.optim.Lars(
                model.parameters(),
                lr=args.lr,
                momentum=args.momentum,
                weight_decay=args.weight_decay,
    ...
    profiling = os.environ.get("PROFILE", "OFF").upper() in [
        "1",
        "Y",
        "ON",
        "YES",
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.converge and (
            ...
            print(
                ...
                epoch,
                "] start time = ",
                time.asctime(time.localtime(epoch_start_time)),
            )
        ...
        if not args.distributed or (args.distributed and args.rank == 0):
            ...
            print(
                ...
                epoch,
                "] end time = ",
                time.asctime(time.localtime(epoch_end_time)),
            )
            print(
                "[info] Epoch[",
                epoch,
                "] consume time = ",
                ((epoch_end_time - epoch_start_time) / 3600.0),
                " hours",
            )
    ...
    def trace_handle(prof):
        ...
        if args.xpu is not None:
            torch.save(
                ...
                "./profiling." + profile_name + ".train.pt",
            )
            prof.export_chrome_trace("./rn50.log.json")
        elif args.gpu is not None:
            torch.save(
                prof.key_averages().table(sort_by="self_cuda_time_total"),
                "./profiling.card." + str(args.xpu) + ".pt",
            )
        else:
            torch.save(
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if i == (args.num_iterations - 1) and args.num_iterations >= 2:
                ...
                print(
                    ...
                    % (
                        args.batch_size,
                        (args.batch_size / (duration_total / (len(val_loader) - 2))),
                        top1.avg,
                        top5.avg,
                    )
                )
