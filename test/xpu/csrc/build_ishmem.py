...
ISHMEM_TARGETS = [
    ...
    ("TokenDispatchIshmem.cpp", "libtoken_dispatch_ishmem.so", "TokenDispatchIshmem"),
    (
...
def build():
    ...
    outputs = []
    for src_name, out_name, label in ISHMEM_TARGETS:
        outputs.append(build_one_ishmem(cfg, ishmem_cfg, src_name, out_name, label))
    return outputs


def clean():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    for _, out_name, _ in ISHMEM_TARGETS:
