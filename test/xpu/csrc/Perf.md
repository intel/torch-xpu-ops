...
### Kernel-only 性能 & 优化前后对比

优化内容：
1. **VEC_SIZE=8 向量化 load/store** — 8× 减少 work items，更宽的内存事务
2. **int32 算术** — 避免 GPU 上昂贵的 64-bit div/mod
3. **预计算 ownership 常量** — base/rem/boundary 不再每 thread 重复计算

dtype=bfloat16, kernel-only 中位数（排除 barrier/copy 开销）：

**2 ranks:**

| Config (tokens_per_rank × hidden) | 原始 kernel | 优化 kernel | 加速 |
|:----------------------------------|:-----------:|:-----------:|:----:|
| 256 × 512, topk=4, E=8           | 0.095 ms    | 0.069 ms    | 1.4× |
| 1024 × 1024, topk=4, E=8         | 0.463 ms    | 0.187 ms    | 2.5× |
| 2048 × 2048, topk=8, E=16        | 3.816 ms    | 0.957 ms    | 4.0× |
| 4096 × 4096, topk=8, E=16        | 14.518 ms   | 4.034 ms    | 3.6× |

**4 ranks:**

| Config (tokens_per_rank × hidden) | 原始 kernel | 优化 kernel | 加速 |
|:----------------------------------|:-----------:|:-----------:|:----:|
| 256 × 512, topk=4, E=8           | 0.131 ms    | 0.073 ms    | 1.8× |
| 1024 × 1024, topk=4, E=8         | 0.823 ms    | 0.288 ms    | 2.9× |
| 2048 × 2048, topk=8, E=16        | 6.497 ms    | 2.111 ms    | 3.0× |
| 4096 × 4096, topk=8, E=16        | 25.992 ms   | 8.939 ms    | 2.9× |

> 大配置下 kernel 实现 **3-4× 加速**；端到端（含 barrier）也有 **2-3× 加速**。
