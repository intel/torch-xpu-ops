...
// The bandwidth-critical 16-byte store pushes a block into the right peer's
// output slot over the cross-GPU link. Use an Intel GPU LSC store with explicit
// cache control (L1WB_L3WB) so contiguous remote writes get combined through L3
// into larger burst transactions (same lever measured ~18% faster on the
// reduce-scatter-unpermute push path). Enabled by default; define
...
struct RingAllgatherSingleKernel {
  ...
  inline void wg_copy2(
    ...
    if ((a % (VEC_SIZE * sizeof(scalar_t))) == 0 && n >= VEC_SIZE) {
      ...
      for (int64_t i = lid; i < nv; i += lsize) {
        const vec_t v = sv[i];
        dv0[i] = v;                  // local own slot (plain)
        ring_vec_store(&dv1[i], v);  // remote right peer (LSC push)
      }
      for (int64_t i = nv * VEC_SIZE + lid; i < n; i += lsize) {
