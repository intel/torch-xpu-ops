...
class TestCudaTrace(TestCase):
    ...
    def test_memory_allocation_callback(self):
        ...
        tensor = torch.empty(10, 4, device=GPU_TYPE)
    ...
    def test_memory_deallocation_callback(self):
        ...
        tensor = torch.empty(3, 8, device=GPU_TYPE)
    ...
    def test_stream_creation_callback(self):
        ...
        if torch.version.hip:
            user_stream = torch.get_device_module(GPU_TYPE).Stream()
            with torch.get_device_module(GPU_TYPE).stream(user_stream):
                torch.ones(5, device=GPU_TYPE)
        else:
            torch.get_device_module(GPU_TYPE).Stream()
