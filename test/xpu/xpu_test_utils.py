...
def launch_test(test_case, skip_list=None, exe_list=None):
    ...
    if skip_list is not None:
        ...
        skip_options += '"'
        test_command = (
            f"pytest --junit-xml=./op_ut_with_skip_{test_case}.xml --max-worker-restart=1000 " + test_case
        )
        ...
        exe_options += '"'
        test_command = (
            f"pytest --junit-xml=./op_ut_with_exe_{test_case}.xml --max-worker-restart=1000 " + test_case
        )
        test_command += exe_options
    else:
        test_command = (
            f"pytest --junit-xml=./op_ut_with_all_{test_case}.xml --max-worker-restart=1000 " + test_case
        )
