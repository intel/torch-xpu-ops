...
Non-primal fwd outputs from model w/o backward hook: {mod_no_hook_fwd_outputs_no_primal}.""",
    ...
    def test_pattern_matcher(self, device):
        ...
        dprops = torch.get_device_module(GPU_TYPE).get_device_properties(device)
        cudnn_version = (
        ...
        if prefer_cudnn and torch.version.get_device_module(GPU_TYPE):
...
cos: aten.cos.default -> PREFER_RECOMPUTE""",
    ...
    def test_region_activation_memory_budget_reduces_act_mem(self):
        ...
        torch._dynamo.reset()
        compiled = torch.compile(Model().to(GPU_TYPE), backend="aot_eager")
        self.assertGreater(get_act_mem(lambda: compiled(x)), 0)

        torch._dynamo.reset()
        compiled = torch.compile(Model(budget=0.0).to(GPU_TYPE), backend="aot_eager")
    ...
    def test_region_activation_memory_budget_per_region(self):
        ...
        torch._dynamo.reset()
        both_save = torch.compile(Model(1.0, 1.0).to(GPU_TYPE), backend="aot_eager")
        mem_both_save = get_act_mem(lambda: both_save(x))

        torch._dynamo.reset()
        a_recomp = torch.compile(Model(0.0, 1.0).to(GPU_TYPE), backend="aot_eager")
        mem_a_recomp = get_act_mem(lambda: a_recomp(x))

        torch._dynamo.reset()
        b_recomp = torch.compile(Model(1.0, 0.0).to(GPU_TYPE), backend="aot_eager")
        mem_b_recomp = get_act_mem(lambda: b_recomp(x))

        torch._dynamo.reset()
        both_recomp = torch.compile(Model(0.0, 0.0).to(GPU_TYPE), backend="aot_eager")
