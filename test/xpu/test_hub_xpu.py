...
class TestHub(TestCase):
    ...
    def _assert_trusted_list_is_empty(self):
        if not os.path.exists(self.trusted_list_path):
            return
    ...
    def test_list_entrypoints(self):
        entry_lists = hub.list(
            "ailzhang/torchhub_example", trust_repo=True, skip_validation=True
        )
    ...
    @retry(Exception, tries=3)
    @patch("torch.hub._validate_not_a_forked_repo")
    def test_load_commit_from_forked_repo(self, mock_validate_not_a_forked_repo):
        mock_validate_not_a_forked_repo.side_effect = ValueError(
    ...
    def test_trust_repo_builtin_trusted_owners(self):
        torch.hub.load(
            "pytorch/vision", "resnet18", trust_repo="check", skip_validation=True
        )
        self._assert_trusted_list_is_empty()
