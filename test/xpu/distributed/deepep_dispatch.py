...
_HAS_EP_COMBINE_REDUCE = False
if _HAS_EP_COMBINE_KERNEL:
        _HAS_EP_COMBINE_LOCAL = hasattr(torch.ops.symm_mem, "ep_combine_local_")
        _HAS_EP_COMBINE_REDUCE = hasattr(torch.ops.symm_mem, "ep_combine_reduce")
...
def deepep_owner_combine(
    ...
    backend_stream: torch.xpu.Stream = None,
        ...
        # The fused ep_combine kernel always writes every token slot (including
        # zeros for tokens with no owned expert), so no pre-zero pass or extra
        # barrier is required before the push.
        ...
        if _HAS_EP_COMBINE_REDUCE:
                torch.ops.symm_mem.ep_combine_reduce(
                        my_recv_buf, output, world_size
                )
        else:
                output.zero_()
