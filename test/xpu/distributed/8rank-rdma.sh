...
export ISHMEM_SYMMETRIC_SIZE="${ISHMEM_SYMMETRIC_SIZE:-1073741824}"
...
export DISPATCH_RDMA_NVL_THREADS="${DISPATCH_RDMA_NVL_THREADS:-384}"
export ISHMEM_IBGDA_QPS_PER_PE="${ISHMEM_IBGDA_QPS_PER_PE:-64}"
...
read -r -a GPU_IDS <<< "${RING_GPU_IDS:-0 1 2 3 4 5 6 7}"
read -r -a NIC_IDS <<< "${RING_NIC_IDS:-0 1 2 3 4 5 6 7}"
if (( ${#GPU_IDS[@]} < 8 || ${#NIC_IDS[@]} < 8 )); then
  echo "RING_GPU_IDS and RING_NIC_IDS must each contain eight entries" >&2
  exit 1
fi
...
echo "[dispatch_rdma_nvl] rank groups: (0,1,2,3) IPC, (4,5,6,7) IPC; cross-group RDMA"
for rank in 0 1 2 3 4 5 6 7; do
...
mpirun -np 8 --prepend-rank bash -c '
