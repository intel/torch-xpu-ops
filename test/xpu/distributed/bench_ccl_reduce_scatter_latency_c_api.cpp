/*
 * bench_ccl_reduce_scatter_latency_c_api.cpp
 *
 * Reduce-Scatter kernel-time latency benchmark using oneCCL **C API** (NCCL-like).
 * Requires oneAPI 2026.0+ (oneCCL 2022.0).
 *
 * C API call:
 *   onecclReduceScatter(send, recv, recvcount, dtype, op, comm, stream)
 *   where recvcount = numel/ws and sendbuf must hold numel total elements.
 *
 * busBW factor: (n-1)/n
 * Size column reports total INPUT bytes (numel * 2).
 *
 * Usage:
 *   mpirun -n 4 ./bench_ccl_reduce_scatter_c_api --min 12 --max 28    # 2^12 to 2^28 elements
 *   mpirun -n 4 ./bench_ccl_reduce_scatter_c_api --sizes 917504,1835008  # exact element counts
 *   mpirun -n 4 ./bench_ccl_reduce_scatter_c_api --sizes-kb 1792,3584    # exact KB sizes
 *   mpirun -n 4 ./bench_ccl_reduce_scatter_c_api --sizes-mb 896,1792     # exact MB sizes
 */
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        std::printf("\n%s\n", sep.c_str());
        std::printf("  Reduce-Scatter Latency Benchmark  (oneCCL C API / barrier timing)\n");
        ...
        std::printf("  per-rank recv chunk = total / %d\n", ws);
    ...
    for (auto numel : size_list) {
        ...
        BenchResult res = bench_loop(q, warmup, loop, MPI_COMM_WORLD,
            ...
            [&]() {
                ONECCL_CHECK(onecclReduceScatter(
                    buf_in, buf_chunk,
    ...
    if (rank == 0) {
        std::printf("\n\n  -- reduce_scatter latency summary (C API / barrier) --\n\n");
        print_table_header();
