 ...
 *
 * Allreduce kernel-time latency benchmark using oneCCL **C API**
 * (NCCL-like onecclXxx functions).
 *
 * Requires oneCCL >= 2022.0 (oneAPI 2026.0).  The C API symbols
 * (onecclAllReduce, onecclCommInitRank, onecclGetUniqueId, ...) are NOT
 * exposed in earlier oneAPI 2025.x releases.
 *
 * Differences vs. bench_ccl_allreduce_latency.cpp (C++ API):
 *   1. Includes <oneapi/ccl.h>  (NOT <oneapi/ccl.hpp>)
 *   2. Communicator init via onecclGetUniqueId + MPI_Bcast +
 *      onecclCommInitRank   (mirrors PyTorch xccl.cpp V2 branch)
 *   3. Device binding via onecclSetDevice(rank_local_idx)
 *   4. Stream is a void* pointing to a sycl::queue
 *   5. **C API does not return a ccl::event** → Method A (ccl_event timing)
 *      is not available; only Method B (barrier-based) remains.
 *
 * Output schema identical to Method B section of the C++ API benchmark
 * so the two reports can be diffed directly.
 *
 * Build
 ...
 *        -lccl -lmpi \
 *        bench_ccl_allreduce_latency_c_api.cpp -o bench_ccl_allreduce_latency_c_api
 *
 * Run (4 GPUs, 4 ranks)
 *   ZE_AFFINITY_MASK=0,1,2,3 CCL_ATL_TRANSPORT=mpi \
 *   mpirun -n 4 ./bench_ccl_allreduce_latency_c_api
 */
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        std::printf("\n%s\n", sep.c_str());
        std::printf("  Allreduce Latency Benchmark  (oneCCL C API / barrier timing)\n");
    ...
    if (rank == 0) {
        std::printf("\n\n  -- allreduce latency summary (C API / barrier) --\n\n");
        print_table_header();
