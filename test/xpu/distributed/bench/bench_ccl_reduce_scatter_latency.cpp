/*
 * bench_ccl_reduce_scatter_latency.cpp
 *
 * Reduce-scatter kernel-time latency benchmark (native C++/SYCL/oneCCL).
 *
 * Measurement strategy
 * ─────────────────────
 *   Mirrors bench_ccl_allgather_latency.cpp.  Differences:
 *     • Calls ccl::reduce_scatter(send_buf, recv_buf, recv_count, ...)
 *       where recv_count = numel / ws (per-rank chunk) and send buffer holds
 *       the full input (numel total bf16 elements).
 *     • busBW correction for reduce_scatter = (n-1)/n  (same as allgather).
 *     • "Size" column reports the **total input size** in bytes
 *       (i.e. numel * sizeof(bf16)), so it can be compared directly with
 *       allgather (output size) and allreduce (message size).  Each rank
 *       actually moves (n-1)/n of that on the wire.
 *
 * CLI / Build / Run:
 *
 ...
 *        -lccl -lmpi \
 *        bench_ccl_reduce_scatter_latency.cpp -o bench_ccl_reduce_scatter_latency
 *
 *   ZE_AFFINITY_MASK=0,1,2,3 CCL_ATL_TRANSPORT=mpi \
 *   mpirun -n 4 ./bench_ccl_reduce_scatter_latency
 */
...
// BusBW correction: reduce_scatter = (n-1)/n
...
// ---------------------------------------------------------------------------
// bench_reduce_scatter: fire-and-forget loop, timing via ccl::event + barrier
// ---------------------------------------------------------------------------
template<typename SubmitFn>
static BenchResult bench_reduce_scatter(sycl::queue& q,
                                        int warmup, int loop,
                                        MPI_Comm comm,
                                        bf16* prefill_buf,
                                        size_t prefill_n,
...
int main(int argc, char** argv) {
    ...
    // ── Allocate buffers ──────────────────────────────────────────────────
    // numel = total input size in bf16 elements; each rank receives numel/ws.
    ...
    if (rank == 0) {
        ...
        std::printf("\n%s\n", sep.c_str());
        std::printf("  Reduce-Scatter Latency Benchmark  "
        ...
        std::printf("  sizes (total input): 2^%d .. 2^%d bf16 elements  (%s .. %s)\n",
                    min_log2, max_log2,
                    fmt_size((double)((int64_t)1<<min_log2)*2).c_str(),
                    fmt_size((double)((int64_t)1<<max_log2)*2).c_str());
        std::printf("  per-rank chunk = total / %d  (recv_count for reduce_scatter)\n", ws);
    ...
    for (int lg = min_log2; lg <= max_log2; lg += step) {
        ...
        const double  bytes = static_cast<double>(numel) * 2.0;  // total input bytes

        BenchResult res = bench_reduce_scatter(q, warmup, loop, MPI_COMM_WORLD,
        ...
        if (rank == 0) {
            ...
            std::printf("\n  [reduce_scatter %12s]  prefill_us:", sz.c_str());
            for (int r = 0; r < ws; ++r) {
