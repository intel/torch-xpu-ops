...
# =============================================================================
# Custom SYCL one-shot / two-shot allreduce (registered by torch-xpu-ops)
# - Op: torch.ops.symm_mem.one_shot_all_reduce  (out-of-place, XPU dispatch)
# - Op: torch.ops.symm_mem.two_shot_all_reduce_ (in-place,    XPU dispatch)
#
# Signal-pad barrier is forced on (USE_SIGNAL_BARRIER=1) because the default
# XCCL-allreduce barrier adds ~35-40us per call, which dominates small-message
# latency on BMG. Signal-pad barrier has been validated on BMG (see §12.2 of
# /data/hanchao/symm/instruction.md).
# =============================================================================

import os as _os
_os.environ.setdefault("USE_SIGNAL_BARRIER", "1")


def _allreduce_custom_impl(
    tensor: torch.Tensor,
    op: str,
    group: dist.ProcessGroup | None,
    kind: str,
) -> torch.Tensor:
    """Common wrapper: stage user tensor into symm buffer, launch op, copy back.

    kind: "one_shot" or "two_shot".
    """
    if op != "sum":
        raise ValueError(f"Only 'sum' is supported, got '{op}'")
    ...
    workspace_bytes = numel * tensor.element_size()
    workspace = symm_mem.get_symm_mem_workspace(group_name, min_size=workspace_bytes)
    symm_buf = workspace.get_buffer(rank, (numel,), tensor.dtype, storage_offset=0)

    symm_buf.copy_(tensor_flat)

    if kind == "one_shot":
        result = torch.ops.symm_mem.one_shot_all_reduce(symm_buf, op, group_name)
        tensor_flat.copy_(result)
    elif kind == "two_shot":
        torch.ops.symm_mem.two_shot_all_reduce_(symm_buf, op, group_name)
        tensor_flat.copy_(symm_buf)
    else:
...
def allreduce_one_shot_custom(
    tensor: torch.Tensor,
    op: str = "sum",
    group: dist.ProcessGroup | None = None,
) -> torch.Tensor:
    """Our custom one-shot allreduce (XPU SYCL kernel, signal-pad barrier)."""
    return _allreduce_custom_impl(tensor, op, group, kind="one_shot")


def allreduce_two_shot_custom(
    tensor: torch.Tensor,
    op: str = "sum",
    group: dist.ProcessGroup | None = None,
) -> torch.Tensor:
    """Our custom two-shot allreduce (XPU SYCL kernel, signal-pad barrier)."""
    return _allreduce_custom_impl(tensor, op, group, kind="two_shot")
