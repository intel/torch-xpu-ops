...
static BenchResult bench_loop(sycl::queue& q,
    ...
    // prefill kernel (keeps GPU busy → more stable timing)
    ...
    uint64_t pf1 = prefill_ev.get_profiling_info<
        sycl::info::event_profiling::command_end>();
    double prefill_us_val = (pf1 >= pf0) ? static_cast<double>(pf1 - pf0) / 1e3 : 0.0;

    return {barrier_us, prefill_us_val};
}
...
int main(int argc, char** argv) {
    ...
    std::vector<int64_t> exact_sizes;
