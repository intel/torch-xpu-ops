 ...
 *
 * Allreduce kernel-time latency benchmark for small messages (native C++/SYCL/oneCCL).
 *
 ...
 *         for i in [0, loop):
 *             ccl_evs[i] = ccl::allreduce(...)   ← store event, no .wait()
 *   4. q.wait()  — single sync point for all GPU work
 *   5. Discard the first (loop - 50) iterations; only use last 50 for stats.
 *   6. Read profiling timestamps from ccl::event's native sycl::event:
 *         per_iter_us[i] = ccl_evs[i].command_end − ccl_evs[i].command_start
 *         span_us        = (ccl_evs[last].command_end −
 *                           ccl_evs[first_measured].command_start) / 50
 ...
 *
 * Key difference from bench_ccl_prefill.cpp:
 *   No bookend kernels (pre/post single_task). Instead we directly use the
 *   sycl::event obtained from ccl::event::get_native() to read
 *   command_start/command_end profiling info for each allreduce invocation.
 *
 * Prefill kernel
 * ──────────────
...
// ---------------------------------------------------------------------------
// bench_allreduce: fire-and-forget loop, timing via ccl::event + barrier kernels
// ---------------------------------------------------------------------------
...
static BenchResult bench_allreduce(sycl::queue& q,
    ...
    for (int i = 0; i < measure; ++i) {
        // Method 1: ccl event native profiling
        ...
        // Method 2: barrier-based (post.command_start - pre.command_end)
...
int main(int argc, char** argv) {
    ...
    // ── Gather buffer: [ccl_per_us[0..m-1], barrier_per_us[0..m-1], prefill_us]
    ...
    struct RowData {
        ...
        // barrier timing
        double b_avg_us, b_min_us, b_max_us, b_var_us;
    };
    ...
    for (int lg = min_log2; lg <= max_log2; lg += step) {
        ...
        BenchResult res = bench_allreduce(q, warmup, loop, MPI_COMM_WORLD,
                                          prefill, prefill_n, prefill_reps,
            [&]() -> ccl::event {
                return ccl::allreduce(
                    buf_in, buf_out, static_cast<size_t>(numel),
