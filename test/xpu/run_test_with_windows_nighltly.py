...
def launch_test(test_case, skip_list=None, exe_list=None):
    ...
    if skip_list is not None:
        if skip_list:
            ...
            skip_options += '"'
            test_command = (
                f"pytest --junit-xml=./op_ut_windows_with_skip.{module_name}.xml " + test_case
            )
            test_command += skip_options
        else:
            test_command = (
                f"pytest --junit-xml=./op_ut_windows_with_all.{module_name}.xml " + test_case
            )
        ...
        if exe_list:
            ...
            exe_options += '"'
            test_command = (
                f"pytest --junit-xml=./op_ut_windows_with_exe.{module_name}.xml " + test_case
            )
