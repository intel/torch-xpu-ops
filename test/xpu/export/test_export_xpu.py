...
def forward(self, x):
    ...
    empty_memory_format = torch.ops.aten.empty.memory_format([10, 8], dtype = torch.float32, device = device(type='xpu', index=0), pin_memory = False)
    ...
    ones_default = torch.ops.aten.ones.default([4], dtype = torch.int64, device = device(type='xpu', index=0), pin_memory = False)
    ...
    return pytree.tree_unflatten((add_tensor,), self._out_spec)""",
            ...
            inp = torch.randn(4, 4, 4, device=device_type)

            # Call container first to move shared weights to device
            export_out = gm(inp)
            eager_out = container_eager(inp)
            self.assertEqual(export_out, eager_out)

            # This should not fail even though weights are now on device
            # and .to(device) returns the same parameter with requires_grad=True
...
def forward(self, x):
    ...
    def test_assert_tensor_metadata_device_index(self):
        ...
        inp = (torch.randn(3, device="xpu"), torch.randn(3, device="xpu"))
        ep = export(N(), inp)
        ep = move_to_device_pass(ep, {"xpu:0": "xpu"})
        ep.module()(torch.randn(3, device="xpu:0"), torch.randn(3, device="xpu:0"))
