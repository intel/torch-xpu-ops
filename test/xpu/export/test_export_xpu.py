...
def forward(self, x):
    ...
    empty = torch.ops.aten.empty.memory_format([10, 8], dtype = torch.float32, device = device(type='xpu', index=0), pin_memory = False)
    ...
    ones = torch.ops.aten.ones.default([4], dtype = torch.int64, device = device(type='xpu', index=0), pin_memory = False)
