...
def forward(self, args_0):
    ...
    def test_aot_export_flex_attention_callable_mask_mod(self):
        ...
        model = FlexAttentionModel(embed_dim, num_heads).to(device_type)
        x = torch.randn(1, seq_len, embed_dim, device=device_type)
...
def forward(self, args_0):
    ...
    def test_dynamo_graph_capture_fx_graph_annotate_overlap_pass(self):
        ...
        def input_fn():
            inputs = (torch.rand(2, 128, device=device_type, requires_grad=True),)
            grad_ins = (torch.rand(2, 128, device=device_type),)
            return (
                *inputs,
                *grad_ins,
            )
    ...
    def test_aot_export_blockmask_with_new_closure(self):
        ...
        args = (torch.randn(2, 128, device=device_type),)
...
def forward(self, arg0_1):
    ...
    def test_aot_export_blockmask_closure_spec_mismatch(self):
        ...
        mask_b = create_block_mask(
            make_mask_fn(8), B=1, H=1, Q_LEN=64, KV_LEN=64, device=device_type
        )
