...
default](args = (%x, %b_state), kwargs = {})
    ...
    @unittest.skipIf(not TEST_GPU, "requires cuda or xpu")
    def test_move_device_to(self):
        class M(torch.nn.Module):
            def forward(self, x):
                x = torch.ops.aten.to.device(
                    x, device=f"{device_type}:0", dtype=torch.float32
                )
                return x + x
...
def forward(self, arg0_1):
    ...
    def test_move_to_device_pass(self):
        ...
        # move the exported program from cpu to the current accelerator
        mod = Model()
        example_inputs = (torch.rand(1, 10, 4),)
        ep = export(mod, example_inputs, strict=True)
        location = torch.device(f"{device_type}:0")
        ep = move_to_device_pass(ep, location=location)
        gm = ep.module()
        test_inputs = (torch.rand(1, 10, 4).to(f"{device_type}:0"),)
        outputs = gm(*test_inputs)
        self.assertEqual(outputs.device, torch.device(f"{device_type}:0"))
        # move it back to cpu
        location = "cpu"
        ep = move_to_device_pass(ep, location=location)
        gm = ep.module()
        ...
        # move it to the current accelerator again
        location = {"cpu": f"{device_type}:0"}
        ep = move_to_device_pass(ep, location=location)
        gm = ep.module()
        test_inputs = (torch.rand(1, 10, 4).to(f"{device_type}:0"),)
        outputs = gm(*test_inputs)
    ...
    def test_move_device_example_inputs(self):
        ...
        # Move to the current accelerator
        location = torch.device(f"{device_type}:0")
