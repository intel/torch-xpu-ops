...
""",
    ...
    @unittest.skipIf(
        not PLATFORM_SUPPORTS_FLASH_ATTENTION_XPU, "XPU flash attention not available"
    )
    def test_sdpa(self):
        @nested_compile_region
        def gn(q, k, v):
        ...
        q = torch.randn(
            1, 1, 32, 32, device="xpu", dtype=torch.bfloat16, requires_grad=True
        )
        k = torch.randn(
            1, 1, 32, 32, device="xpu", dtype=torch.bfloat16, requires_grad=True
        )
...
""",
    ...
    def test_side_effect_with_aliased_intermediate(self):
        ...
        with self.assertRaisesRegex(
            ...
            "An intermediate tensor 'view' created inside a higher-order op subgraph aliases an input or output",
        ):
            opt_fn(x)
...
""",
    ...
    def test_return_none(self):
        ...
        weight = torch.ones(
            1000, device="xpu:0", dtype=torch.float32, requires_grad=True
        )
        ones = torch.ones(1000, device="xpu:0", dtype=torch.float32)
