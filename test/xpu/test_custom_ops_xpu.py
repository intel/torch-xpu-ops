...
Dynamic shape operator
 ...
 For more details about this graph break, please visit: https://meta-pytorch.github.io/compile-graph-break-site/gb/gb0036.html""",
    ...
    def test_impl_device_cpu(self):
        ...
        self._test_impl_device("foo3", ["cpu", "xpu"], "cpu")
...
"""
    ...
    def test_library_register_torch_dispatch_low_level(self):
        ...
        device_types_options = [("cpu", "xpu"), "cpu", None]
    ...
    def test_library_register_kernel(self):
        ...
        for mode, call, device_types in itertools.product(
            ...
            @torch.library.custom_op(
                "_torch_testing::add", mutates_args=(), device_types="xpu"
            )
            def add(x: Tensor, y: float) -> Tensor:
                x_np = x.cpu().numpy()
                out_np = x_np + y
    ...
    def test_library_register_autocast_multiple_times_different_devices(self):
        ...
        # Register XPU autocast for the second time
        torch.library.register_autocast(my_sin, "xpu", torch.float16)
...
Please use `add.register_fake` to add an fake impl.""",
    ...
    @skipIfTorchDynamo("Expected to fail due to no FakeTensor support; not a bug")
    @unittest.skipIf(not TEST_XPU, "requires XPU")
    def test_split_device(self):
        ...
        @f.register_kernel("xpu")
        def _(x: Tensor) -> Tensor:
            nonlocal xpu_call_count
            xpu_call_count += 1
            x_np = x.cpu().numpy()
            out_np = np.sin(x_np)
            return torch.from_numpy(out_np).to(x.device)
        ...
        self.assertEqual(xpu_call_count, 0)

        x = x.xpu()
        y = f(x)
        self.assertEqual(y, x.sin())
        self.assertEqual(cpu_call_count, 1)
        self.assertEqual(xpu_call_count, 1)

    @skipIfTorchDynamo("Expected to fail due to no FakeTensor support; not a bug")
    @unittest.skipIf(not TEST_XPU, "requires XPU")
    def test_multi_types(self):
        @torch.library.custom_op(
            "_torch_testing::f", mutates_args=(), device_types=("cpu", "xpu")
        )
        def f(x: Tensor) -> Tensor:
            x_np = x.cpu().numpy()
            out_np = np.sin(x_np)
    ...
    def test_factory_function(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "f does not have a kernel registered for xpu"
        ):
            f("xpu")
    ...
    def test_opcheck_customopdef(self):
        ...
        if torch.xpu.is_available():
            sample_inputs.extend(
                [
                    (torch.randn(3, device="xpu"),),
                    (torch.randn(3, device="xpu", requires_grad=True),),
                ]
            )
