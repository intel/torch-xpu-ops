...
class TestFlopCounter(TestCase):
    ...
    def test_sdpa_nested_tensor(self):
        ...
        def get_nested_inputs(
            ...
            q_offsets, k_offsets = (
                torch.cat((torch.tensor([0]), torch.cumsum(lengths, dim=0))).to("xpu")
    ...
    def test_nested_attention_fake_tensors(self):
        ...
        dense_x = torch.randn(
            4, 40, 4, 16, dtype=torch.bfloat16, device="xpu"
        ).transpose(1, 2)
    ...
    def test_flop_counter_custom_triton_op_two_kernels_auto_ac(self):
        ...
        n_elements = int(1e7)
        x = torch.randn(n_elements, device="xpu", requires_grad=True)
    ...
    def test_scaled_mm(self):
        ...
        with FlopCounterMode() as mode:
            torch._scaled_mm(
                torch.randn((3 * 16, 5 * 16), device="xpu").to(dtype),
                torch.randn((7 * 16, 5 * 16), device="xpu").to(dtype).t(),
                scale_a=torch.ones((), device="xpu"),
                scale_b=torch.ones((), device="xpu"),
