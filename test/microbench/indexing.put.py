...
def benchmark(args):
    for shape in shape_list:
        for dtype in [torch.bfloat16, torch.float16, torch.float32]:
            ...
            indices = torch.linspace(0, 8190 * 8190, steps=4096 * 4096, device=args.device).to(
                torch.long
            )
            ...
            if not args.e2e_only:
                run_profile(input, indices, sources, cache_r, cache_w, args.device, args.num_iter)

            if not args.profile_only:
                run_e2e(input, indices, sources, cache_r, cache_w, args.device, args.num_iter)
