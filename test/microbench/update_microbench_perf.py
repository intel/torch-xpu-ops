...
def replace_time_columns_inplace(main_file, source_file, backup=True):
    ...
    try:
        ...
        )  # dtype=str to preserve exact string matching
        ...
        if missing_time_main:
            raise ValueError(
                f"Missing time columns in {main_file}: {missing_time_main}"
            )
        if missing_time_source:
            raise ValueError(
                f"Missing time columns in {source_file}: {missing_time_source}"
            )
        ...
        missing_keys_source = [
            col for col in key_columns if col not in df_source.columns
        ]
        if missing_keys_source:
            raise ValueError(
                f"Missing key columns in {source_file}: {missing_keys_source}"
            )

        print(f"\n🔑 Using {len(key_columns)} key columns for matching:")
        print(
            "  " + ", ".join(key_columns[:5]) + ("..." if len(key_columns) > 5 else "")
        )
        ...
        if duplicated_keys > 0:
            print(
                f"⚠️  Warning: {duplicated_keys} duplicate key(s) found in source file. First occurrence used."
            )
        ...
        for idx in range(len(df_main)):
            ...
            if key in source_dict:
                ...
                if replaced_count <= 3:  # log first 3
                    print(
                        f"🔄 Row {idx}: time({old_time} → {new_time}), E2E({old_e2e} → {new_e2e})"
                    )
...
def main():
    ...
    parser.add_argument(
        "source_file", help="Source CSV file (provides new time values)"
    )
    parser.add_argument(
        "--no-backup", action="store_true", help="Skip backup (⚠️ risky!)"
    )
