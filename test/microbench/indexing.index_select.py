...
def Index_select(shape, dtype, backward, device):
    main_size = shape[0]
...
def run_profile(shape, dtype, backward, cache_r, cache_w, device, num_iter):
    with profile(
        ...
        for i in range(num_iter):
            cache_r = cache_w * i
            Index_select(shape, dtype, backward, device)
    print(prof.key_averages().table(sort_by="{}_time_total".format(device)))

def run_e2e(shape, dtype, backward, cache_r, cache_w, device, num_iter):
...
def benchmark(args):
    for shape in shape_list:
        for dtype in [torch.bfloat16, torch.float16, torch.float32]:
            ...
            # warm up
            Index_select(shape, dtype, backward, args.device)

            # go
            print("shape:", (shape), "; datatype:", dtype, "; backward:", backward)
            if not args.e2e_only:
                run_profile(shape, dtype, backward, cache_r, cache_w, args.device, args.num_iter)

            if not args.profile_only:
                run_e2e(shape, dtype, backward, cache_r, cache_w, args.device, args.num_iter)
