...
def AVGPool3d(shape, dtype, channels_last, backward):
    N, C, D, H, W, kernel_size, stride = shape[0], shape[1], shape[2], shape[3], shape[4], shape[5], shape[6]
    ...
    for channels_last in [False, True]:
        for shape in shape_list:
            for dtype in [torch.bfloat16, torch.float16, torch.float32]:
                ...
                # go
                print("shape:", (shape[0], shape[1], shape[2], shape[3], shape[4]), "; datatype:", dtype, "; kernel_size:", shape[5], "; stride:", shape[6], "; channels_last:", channels_last, "; backward:", backward)
