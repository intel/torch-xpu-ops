    ...
    for shape in shape_list:
        for dtype in [torch.bfloat16, torch.float16, torch.float32]:
            for dim in [0, 1]:
                ...
                indices = torch.linspace(0, 1022, steps=step, device=args.device).to(torch.long)
                ...
                if not args.e2e_only:
                    run_profile(input, indices, y_0, y_1, dim, cache_r, cache_w, args.device, args.num_iter)

                if not args.profile_only:
                    run_e2e(input, indices, y_0, y_1, dim, cache_r, cache_w, args.device, args.num_iter)
