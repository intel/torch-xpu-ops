            ...
            for mode in ["bilinear", "nearest"]:
                for padding_mode in ["zeros", "border", "reflection"]:
                    for align_corners in [True, False]:
                        N, C, D, H, W = shape
                        input = torch.randn(N, C, D, H, W, dtype=dtype, device=args.device)
                        grid = torch.randn(N, D, H, W, 3, dtype=dtype, device=args.device)

                        if backward:
                            input.requires_grad_(True)
                            grid.requires_grad_(True)
                        # warm up
                        Grad_sample3d(input, grid, backward, mode, padding_mode, align_corners, args.device)
