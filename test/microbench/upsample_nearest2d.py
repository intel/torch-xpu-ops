...
def Interpolate2d(shape, dtype, channels_last, backward):
    ...
    output = torch.nn.functional.interpolate(input, scale_factor=shape[4], mode='nearest')
    ...
    for channels_last in [False, True]:
        for shape in shape_list:
            for dtype in [torch.bfloat16, torch.float16, torch.float32]:
                # warm up
                Interpolate2d(shape, dtype, channels_last, backward)

                # go
                print("shape:", (shape[0], shape[1], shape[2], shape[3]), "; datatype:", dtype, "; scale_factor:", shape[4], "; channels_last:", channels_last, "; backward:", backward)
