...
def _make_noncontiguous(t):
    ...
    big = torch.empty(
        t.numel() * 2, dtype=t.dtype, device=t.device
    )
    view = big[::2]
...
def test_batch_norm_legit_noncontiguous_save_mean_out():
    """
    ...
    save_mean_nc = _make_noncontiguous(torch.zeros(C, dtype=acc_dtype, device=device))
    save_invstd_nc = _make_noncontiguous(torch.zeros(C, dtype=acc_dtype, device=device))
    out_nc = torch.empty_like(inp)

    assert not save_mean_nc.is_contiguous(), "save_mean should be non-contiguous"
    assert not save_invstd_nc.is_contiguous(), "save_invstd should be non-contiguous"
...
def test_batch_norm_stats_noncontiguous_output():
    """
    batch_norm_stats (the standalone stats kernel) should also work when the
    ...
    # Simply verify shapes and dtypes — the main goal is no crash.
    assert mean.shape == torch.Size([C])
    assert invstd.shape == torch.Size([C])
