...
int main(int argc, char *argv[]) {
    const size_t count = 200 * 1024 * 1024;
    ...
    for (int i = 0; i < 10; i ++) {
        for (int j = 0; j < 1; j ++) {
          ...
          auto kernel2_depend_event = queue2.memcpy(compute_buf2, recv_bufs2, count); // allgather
          // compute kernels
	      queue2.submit([&](sycl::handler &cgh) {
	        cgh.depends_on(event1);
            cgh.parallel_for<class communication_kernel>(sycl::nd_range<1>(num_wg * wg_size, wg_size),
                    [=](sycl::nd_item<1> item) {
                    int id = item.get_global_id();
                    if (id >= max) return;
                    recv_bufs1[id] += id * 0.00001;
            });
          });
          queue2.memcpy(compute_buf1, recv_bufs1, count);   // reduce scatter

          queue1.ext_oneapi_submit_barrier({kernel2_depend_event});
