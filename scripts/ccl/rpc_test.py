...
class HybridModel(torch.nn.Module):
   ...
   """
    ...
    def __init__(self, emb_rref_list, device):
        ...
        fc1 = torch.nn.Linear(512, 256).to("xpu:"+str(device))
        fc2 = torch.nn.Linear(256, 128).to("xpu:"+str(device))
        relu = torch.nn.ReLU().to("xpu:"+str(device))
        fc3 = torch.nn.Linear(128, 64).to("xpu:"+str(device))
        fc4 = torch.nn.Linear(64, 32).to("xpu:"+str(device))
        fc5 = torch.nn.Linear(32, 8).to("xpu:"+str(device))
        sec = nn.Sequential(fc1, fc2, relu, fc3, fc4, fc5).to("xpu:"+str(device))
        self.ddp = DDP(sec, device_ids=[device])
        self.device = device
