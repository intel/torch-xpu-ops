...
name: issue-format
description: >
  Extract metadata from a pre-classified issue.
  The format agent has already split the body into sections and classified them.
  The LLM only extracts metadata (test_type, category, dependency, platform, context)
  and determines issue_type (bug vs nonbug).
---

# Issue Format — Extract Metadata

## Your Task

Given a GitHub issue that has already been split into sections (description,
reproducer, error_log, environment, additional_context), extract metadata.
...
7. Keep the output small. Do NOT echo back the issue body.
