...
- NEVER modify files outside your repo scope. Exception: if you're in the pytorch repo and the issue targets `third_party/torch-xpu-ops/` files, you may edit those (they are torch-xpu-ops source files bundled as a submodule). Do NOT modify other `third_party/*` submodules. If in torch-xpu-ops: only modify files under `src/`.
- NEVER modify unrelated files.
