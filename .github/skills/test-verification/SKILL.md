...
The working directory is always `~/pytorch/`. All test paths must be relative to it.
...
2. **Bare filename** (e.g., `test_ops.py` with no directory) → Search the filesystem:
   ```bash
   find ~/pytorch/ -name "test_ops.py" -not -path "*/.git/*" -not -path "*/build/*" -not -path "*/.venv/*" -not -path "*__pycache__*" | head -20
   ```
   - If the issue is about XPU and multiple matches exist, prefer files under `third_party/torch-xpu-ops/` or `test/xpu/`
   - If only one match → use it

3. **Upstream pytorch test paths** (e.g., `test/nn/test_embedding.py`, `test/inductor/test_cuda_repro.py`) → Use as-is, they are relative to `~/pytorch/`
...
grep "class TestSparseAny" ~/pytorch/third_party/torch-xpu-ops/test/xpu/test_sparse_xpu.py
# Check if instantiate_device_type_tests is called
grep "instantiate_device_type_tests" ~/pytorch/third_party/torch-xpu-ops/test/xpu/test_sparse_xpu.py
```
...
```bash
cd ~/pytorch && pytest --collect-only -q "<file>" -k "<filter>" 2>&1 | tail -5
```
