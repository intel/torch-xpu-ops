...
ZE_AFFINITY_MASK=${CARD} \
    eval python -u benchmarks/dynamo/"${SUITE}".py --"${SCENARIO}" --"${Real_DT}" -d "${DEVICE}" -n10 "${DT_extra}" "${Mode_extra}" \
    "${Shape_extra}" "${partition_flags}" "${Model_only_extra}" --backend=inductor --cold-start-latency --timeout=10800 --disable-cudagraphs \
         --output="${LOG_DIR}"/"${LOG_NAME}".csv 2>&1 | awk '{ print strftime("[%Y-%m-%d %H:%M:%S]"), $0 }' | tee -a "${LOG_DIR}"/"${LOG_NAME}"_card"${CARD}".log
