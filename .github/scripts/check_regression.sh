...
setup_conda_env() {
    ...
    conda create --name "${env_name}" --clone base -y || {
        conda create --name "${env_name}" python=$("${PYTHON_EXECUTABLE}" -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')") -y
    }

    # Save environment spec
    conda list --name "${env_name}" > "${BASELINE_ENV_DIR}/conda_packages.txt"
...
setup_venv() {
    ...
    "${PYTHON_EXECUTABLE}" -m pip freeze > "${BASELINE_ENV_DIR}/requirements.txt"
...
rerun_baseline() {
    ...
    if [[ -z "${BASELINE_PYTHON}" ]]; then
        log_error "Baseline Python not set. Environment setup may have failed."
        return 1
    fi
...
check_average_regression() {
    ...
    # Count regressions
    local total=$(awk 'NR>1 {count++} END {print count}' "${comparison_file}")
    local regressions=$(awk -F',' 'NR>1 && ($9 == "YES" || $10 == "YES") {count++} END {print count}' "${comparison_file}")
...
generate_final_report() {
...

    {
        ...
        if [[ "${REGRESSION_CONFIRMED}" == "true" ]]; then
            echo "❌ **REGESSIONS CONFIRMED** - Performance degradation detected"
        else
        ...
        if [[ -f "${comparison_file}" ]]; then
            ...
            echo "- Models with regression: ${regressions}"
            echo "- Regression rate: $(awk "BEGIN {printf \"%.1f%%\", ${regressions}/${total}*100}")"
            echo ""
        ...
        if [[ "${REGRESSION_CONFIRMED}" == "true" ]]; then
            ...
            echo "3. Review environment differences"
    ...
    local summary_file="${OUTPUT_DIR}/summary.txt"
    {
        ...
        echo "Total models tested: $(awk 'NR>1 {count++} END {print count}' "${comparison_file}" 2>/dev/null || echo "N/A")"
        echo "Regressions confirmed: ${regressions:-0}"
...
# Include your existing helper functions here (keeping them as is)
# process_summary_file, analyze_performance_regression, etc.
