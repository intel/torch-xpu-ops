...
if [[ "${ut_suite}" == 'op_regression' || "${ut_suite}" == 'op_regression_dev1' || "${ut_suite}" == 'op_extended' || "${ut_suite}" == 'op_transformers' ]]; then
    grep -E "^FAILED|have failures" "${ut_suite}"_test.log | sed 's/\- .*//' > ./"${ut_suite}"_failed.log
...
if [[ "${ut_suite}" == 'op_ut' ]]; then
    grep -E "^FAILED|have failures" op_ut_with_skip_test.log | sed 's/\- .*//' > ./"${ut_suite}"_with_skip_test_failed.log
    grep -E "^FAILED|have failures" op_ut_with_only_test.log | sed 's/\- .*//' > ./"${ut_suite}"_with_only_test_failed.log
...
if [[ "${ut_suite}" == 'torch_xpu' ]]; then
    ...
    for xpu_case in build/bin/*{xpu,sycl}*; do
      if [[ "$xpu_case" != *"*"* && "$xpu_case" != *.so && "$xpu_case" != *.a ]]; then
        ...
        grep -E "FAILED|have failures" binary_ut_"${ut_suite}"_"${case_name}"_test.log | sed 's/\- .*//' > ./binary_ut_"${ut_suite}"_"${case_name}"_failed.log
        wc -l < "./binary_ut_${ut_suite}_${case_name}_failed.log" | tee -a ./binary_ut_"${ut_suite}"_failed_summary.log
        grep -E "PASSED|Pass" binary_ut_"${ut_suite}"_"${case_name}"_test.log | sed 's/\- .*//' > ./binary_ut_"${ut_suite}"_"${case_name}"_passed.log
...
if [[ "${ut_suite}" == 'xpu_distributed' ]]; then
    grep -E "^FAILED|have failures" xpu_distributed_test.log | sed 's/\- .*//' > ./"${ut_suite}"_xpu_distributed_test_failed.log
