...
- Match on the test case first, then on the error message or the traced root
  cause. Two of the three signals must agree before claiming a duplicate.
- String-match the returned `body` for the literal full test name, and for a
  ...
  looks unrelated.
- Run one search set per `test_cases[]` entry and merge the results.
...
Cite the matching issue URL and state which signals agreed. A shared operator
name or a generic `RuntimeError` alone is never sufficient.
