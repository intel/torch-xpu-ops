...
and what verification showed. No bullets — this is the part read without
clicking anything.>
...
scan the table to see whether anything needs them. The summary ends with
the `_Generated by [fix job](<run url>)._` sign-off the workflow prompt
hands you — it must stay the comment's last line, ahead of the review
block the workflow appends.
