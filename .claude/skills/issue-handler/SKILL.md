...
In pipeline mode the two blocks below are yours (the leaf skills own
theirs); append them per the collapse rule in
[execution-modes.md](references/execution-modes.md).
...
| Sub-item | Outcome | Branch / Reason |
|---|---|---|
| 4. `test_foo_xpu_float8_e4m3fn` | FIXED | `agent/fix-issue-5272-4-test_foo` |
...
- **Branch:** `agent/fix-issue-<N>[-<sub-item>-<slug>]`
...
The tally bullets list every outcome that occurred and end with a single
`none.` line for the ones that did not, so the reader does not have to
scan the table to see whether anything needs them.

Both blocks are collapsed like every other one, so the `_Generated by
[fix job](<run url>)._` sign-off the workflow prompt hands you goes
**after** the summary's `</details>`, not inside it: it is the reader's
one click to the run and its patch artifact. It stays the comment's last
line, ahead of the review block the workflow appends. The Stage 1 intro
line is then the only uncollapsed text in the comment, so its
`**Outcome:**` is the only place a reader sees how the run ended — always
write it.
