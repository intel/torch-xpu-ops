  ...
  ```json
  {
    ...
    "sub_items": [
      ...
      { "seq": 2, "slug": "test_baz", "outcome": "NEEDS_HUMAN",
        ...
        "reason_detail": "needs a oneDNN change; pytorch + torch-xpu-ops together is not this" },
...
- **`fix_result-<slug>.json`** — for each `FIXED` sub-item, the same
  ...
  - `depends_on` — the fix_result file that must land BEFORE this one, or
    absent. Set it on the companion unit of a two-repo fix (below); the
    patch export names directories `<branch>-<target_repo>`, so the two
    halves are separate series and nothing in the artifact says which
    order they go in except this key.
  plus `needs_build`, `refined_command`, `notes`. Suffixed by slug so each
  sub-item can be exported / re-verified independently.

**A fix that spans both repos** is two units, not a `NEEDS_HUMAN`: the
pytorch clone carries torch-xpu-ops as its submodule, so both halves are
committed and verified in the same build (`fix-root-cause` returns
`target_repo` + `companion_repo`, lands-first repo first). Record it as:

- one branch per repo, each named after the issue as usual — the same name
  in both repos is fine, they are different repositories,
- one fix_result per repo: the companion's file name is the primary's plus
  `-<target_repo>` (`fix_result-grid_sample.json` →
  `fix_result-grid_sample-pytorch.json`),
- `depends_on` on the companion, naming the primary's file,
- the same `verdict` on both, since one build verified the pair. Say so in
  `notes` so nobody reads `PASSED` as "this half was verified alone".
- in `batch_summary.json`, the sub-item's `fix_result` points at the unit
  that fixes the test; the companion is reached through `depends_on`. Do
  not invent extra top-level keys for it.
...
- `READY(reason=ok)` → **commit the staged fix onto the branch created in
  ...
  **When Stage 3 returned a `companion_repo`**, do the same thing twice,
  `target_repo` first: call `fix-implement` again with
  `target_repo_dir` set to the companion checkout, branch it off that
  repo's own HEAD, commit, and write its record as the primary's file
  name plus `-<target_repo>` with `depends_on` naming the primary (see
  "Machine-readable outputs"). Two invocations, one per repo — the skill
  only ever stages one repo's diff. Stage 5 then verifies the pair in one
  build, since the pytorch tree already has torch-xpu-ops as its
  submodule, and both records take that one verdict.
...
A two-repo fix is verified **once, as a pair**: both branches are already
checked out in the same tree, so one build covers them. Update both
records with that verdict, and say in each `notes` that the pair was
verified together, so nobody reads a lone `PASSED` as "this half stands
on its own".
...
**A two-repo fix adds one more thing, for whoever opens the PR** — the
artifact holds two patch series and applying them in the wrong order breaks
CI, and nothing outside this comment says so. Put the ordered instructions
at the top of the Summary body:

```markdown
> [!IMPORTANT]
> **Two PRs, in this order.**
> 1. `<lands-first repo>` — patch dir `<branch>-<repo>`. Open and land first.
> 2. `<companion repo>` — patch dir `<branch>-<repo>`. Only after <what it
>    waits on, e.g. the `xpu.txt` pin bump that carries step 1>.
```

and repeat the one-line version **outside** `</details>`, above the
`_Generated by_` footer, since the Summary block renders collapsed and a
notice nobody expands is a notice nobody reads:
