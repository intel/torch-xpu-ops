...
Only two things stay outside a `<details>`, because they are what the
issue page should show without a click: the Stage 1 intro line, and the
`_Generated by [fix job](<url>)._` sign-off, which goes after the
summary's `</details>` so the run and its patch artifact stay one click
from the issue.

The closing summary being collapsed is why the intro line has to carry
the `**Outcome:**` — collapse it too and the comment would say nothing
