...
- [ ] **Pointwise kernel iteration and launch range**
  ...
  - [ ] Grid-stride pointwise launches bound `global_range` by resident capacity with `syclLoopGroupRange(static_cast<int64_t>(tensor.numel()), work_group_size)`, rather than calculating one work-item per element with `ceil_div(numel, work_group_size)`
