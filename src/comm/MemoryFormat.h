...
inline c10::MemoryFormat cat_compute_output_memory_format(
  ...
  for (auto mt : tensors) {
    auto t = mt.get();
    auto f = suggest_memory_format_sycl(t, channels_last_strides_exact_match);
...
inline Tensor empty_like_sycl(
    const Tensor& self,
    const TensorOptions& options,
    c10::optional<c10::MemoryFormat> optional_memory_format) {
  auto ndim = self.ndimension();
  if (3 != ndim || CHANNELSLAST1D_SYCL != optional_memory_format) {
