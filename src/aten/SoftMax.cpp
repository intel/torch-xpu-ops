...
Tensor& XPUNativeFunctions::_softmax_out(
  ...
  torch_meta_func__softmax(out, self, dim, half_to_float);
  Tensor out_cpu = out.to(Device(kCPU));
  Tensor self_cpu = self.to(Device(kCPU));
  // std::cout << self_cpu.data() << std::endl;
  at::_softmax_out(out_cpu, self_cpu, dim, half_to_float);
  // std::cout << out_cpu.data() << std::endl;
  out.copy_(out_cpu);
  return out;
}

void torch_meta_func__softmax_backward_data(
  ...
  at::xpu::resize_out(grad_input, grad_output.sizes(), {}, grad_input_options);
}
...
Tensor& XPUNativeFunctions::_softmax_backward_data_out(
  ...
  torch_meta_func__softmax_backward_data(
