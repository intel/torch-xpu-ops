...
void fake_quantize_tensor_cachemask_tensor_qparams_kernel(
    ...
    int64_t quant_min,
    int64_t quant_max) {
  float* scale_ptr = scale.const_data_ptr<float>();
  int32_t* zp_ptr = zero_point.const_data_ptr<int32_t>();
  int64_t* fake_quant_on = fake_quant_enabled.const_data_ptr<int64_t>();
  auto iter = TensorIteratorConfig()
                  .check_all_same_dtype(false)
                  .add_output(output)
                  .add_output(mask)
