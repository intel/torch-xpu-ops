...
#define SYCL_TLA_CHECK(status)                                                 \
  do {                                                                         \
    ...
    TORCH_CHECK(                                                               \
        ...
        "sycl-tla error: ", cutlassGetStatusString(_err),                      \
...
static void run_sycl_tla_gemm_impl(
    const void* input_ptr, const void* weight_ptr, void* output_ptr,
    int M, int N, int K, sycl::queue& queue)
{
    ...
    typename Gemm::GemmKernel::Arguments arguments{
        ...
        {{ElementAccumulator(1.0f), ElementAccumulator(0.0f)},
         nullptr, stride_C, D_ptr, stride_D},
        hw_info
    };
    ...
    TORCH_CHECK(
        ...
        "fused_gate_up_silu: problem size M=", M, " N=", N, " K=", K,
...
static void launch_silu_mul_impl(
    const void* combined_ptr, void* output_ptr,
    int M, int N, sycl::queue& queue)
{
    ...
    const at::Tensor& up_weight)
{
    ...
    TORCH_CHECK(dtype == at::kHalf || dtype == at::kBFloat16,
                "Only fp16/bf16 supported, got ", dtype);
    ...
    if (dtype == at::kHalf) {
        dispatch_sycl_tla_gemm<cutlass::half_t>(
            input.data_ptr(), fused_weight.data_ptr(),
            combined.data_ptr(), M, 2*N, K, queue);
