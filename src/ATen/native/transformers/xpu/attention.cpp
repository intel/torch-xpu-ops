...
// contiguous
std::tuple<Tensor, Tensor, Tensor> _transform_bias_rescale_qkv_xpu(
    const Tensor& qkv,
...
std::tuple<Tensor, Tensor> _native_multi_head_attention_xpu(
    const Tensor& query,
    const Tensor& key,
    const Tensor& value,
  ...
  auto q_k_v = _transform_bias_rescale_qkv_xpu(qkv, qkv_bias, num_head);
