...
public:
    void operator()(sycl::nd_item<1> item) const {
        ...
        if (idx < batch_size_ * seq_len_q_ * num_head_q_ * head_dim_v_) {
            static_cast<float*>(output_)[idx] = 10.0f;
        }

    }
    vector_add(int batch_size,
               int seq_len_q,
               int seq_len_kv,
               int num_head_q,
               int num_head_kv,
               ...
               const void* key,
               const void* value,
               void* output) : batch_size_(batch_size),
                           seq_len_q_(seq_len_q),
                           seq_len_kv_(seq_len_kv),
                           num_head_q_(num_head_q),
                           num_head_kv_(num_head_kv),
                           head_dim_qk_(head_dim_qk),
...
void cutlass_sdpa(
    ...
    //                      value,
    //                      output);
    // int total_threads = batch_size * seq_len_q * num_head_q * head_dim_v;
    // int local_range = 256;
    // int global_range = ((total_threads + local_range - 1) / local_range) * local_range;
    // sycl::queue q;
    // q.submit([&](sycl::handler &cgh) {
    //     cgh.parallel_for(sycl::nd_range<1>(sycl::range<1>(global_range),
