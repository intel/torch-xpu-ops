...
Tensor& XPUNativeFunctions::glu_out(
  ...
  native::xpu::glu_kernel(iter, dim);
  return out;
}
...
Tensor& XPUNativeFunctions::glu_backward_out(const Tensor& grad_output, const Tensor& input,
  ...
  if (iter.can_use_32bit_indexing()) {
    glu_backward_kernel(iter, gI_stride, I_stride);
  } else {
    for (const auto& sub_iter: iter.with_32bit_indexing()) {
      glu_backward_kernel(sub_iter, gI_stride, I_stride);
    }
  }
  return grad_input;
}

Tensor XPUNativeFunctions::glu_backward(const Tensor& grad_output, const Tensor& input, int64_t dim) {
