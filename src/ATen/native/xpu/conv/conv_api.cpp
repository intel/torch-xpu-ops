...
at::Tensor conv2d_fprop_impl(
    sycl::queue& queue, const at::Tensor& input, const at::Tensor& weight,
    const at::Tensor& bias, bool has_bias,
    int strideH, int strideW, int padH, int padW, int dilH, int dilW,
    int64_t groups);

at::Tensor conv2d_dgrad_impl(
    sycl::queue& queue, const at::Tensor& grad_output, const at::Tensor& weight,
    int H, int W,
    int strideH, int strideW, int padH, int padW, int dilH, int dilW,
    int64_t groups);

at::Tensor conv2d_wgrad_impl(
    sycl::queue& queue, const at::Tensor& grad_output, const at::Tensor& input,
    int kH, int kW,
    int strideH, int strideW, int padH, int padW, int dilH, int dilW,
    int64_t groups);
...
#define CONV_CHECK_DTYPE(name, t) \
  TORCH_CHECK((t).dtype() == at::kBFloat16 || (t).dtype() == at::kHalf, \
      "sycltla::" name ": only bf16/fp16 supported, got ", (t).dtype())
...
at::Tensor conv2d_fprop(
  ...
  TORCH_CHECK(input.dtype() == weight.dtype(), "sycltla::conv2d_fprop: dtype mismatch");
  ...
  return sycltla::conv2d_fprop_impl(queue, input, weight, bias_tensor, has_bias,
      (int)stride[0], (int)stride[1], (int)padding[0], (int)padding[1],
      (int)dilation[0], (int)dilation[1], groups);
#endif
}
...
at::Tensor conv2d_dgrad(
  ...
  auto& queue = at::xpu::getCurrentXPUStream().queue();
  return sycltla::conv2d_dgrad_impl(queue, grad_output.contiguous(), weight.contiguous(),
      (int)input_size[2], (int)input_size[3],
      (int)stride[0], (int)stride[1], (int)padding[0], (int)padding[1],
      (int)dilation[0], (int)dilation[1], groups);
#endif
}
...
at::Tensor conv2d_wgrad(
  ...
  auto& queue = at::xpu::getCurrentXPUStream().queue();
  return sycltla::conv2d_wgrad_impl(queue, grad_output.contiguous(), input.contiguous(),
...
at::Tensor conv3d_fprop(
  ...
  TORCH_CHECK(input.is_contiguous() && weight.is_contiguous());
  TORCH_CHECK(stride.size() == 3 && padding.size() == 3 && dilation.size() == 3);
  TORCH_CHECK(input.size(1) == weight.size(1) * groups);

  auto& queue = at::xpu::getCurrentXPUStream().queue();
  bool has_bias = bias.has_value() && bias->defined();
  at::Tensor bias_tensor = has_bias ? *bias : at::Tensor();
  return sycltla::conv3d_fprop_impl(queue, input, weight, bias_tensor, has_bias,
      (int)stride[0], (int)stride[1], (int)stride[2],
      (int)padding[0], (int)padding[1], (int)padding[2],
      (int)dilation[0], (int)dilation[1], (int)dilation[2], groups);
#endif
}
...
at::Tensor conv3d_dgrad(
  ...
  auto& queue = at::xpu::getCurrentXPUStream().queue();
  return sycltla::conv3d_dgrad_impl(queue, grad_output.contiguous(), weight.contiguous(),
      (int)input_size[2], (int)input_size[3], (int)input_size[4],
      (int)stride[0], (int)stride[1], (int)stride[2],
      (int)padding[0], (int)padding[1], (int)padding[2],
      (int)dilation[0], (int)dilation[1], (int)dilation[2], groups);
#endif
...
at::Tensor conv3d_wgrad(
  ...
  auto& queue = at::xpu::getCurrentXPUStream().queue();
  return sycltla::conv3d_wgrad_impl(queue, grad_output.contiguous(), input.contiguous(),
...
TORCH_LIBRARY(sycltla_conv, m) {
  m.def("conv2d_fprop(Tensor input, Tensor weight, Tensor? bias, int[] stride, int[] padding, int[] dilation, int groups) -> Tensor");
  m.def("conv2d_dgrad(Tensor grad_output, Tensor weight, int[] input_size, int[] stride, int[] padding, int[] dilation, int groups) -> Tensor");
  m.def("conv2d_wgrad(Tensor grad_output, Tensor input, int[] kernel_size, int[] stride, int[] padding, int[] dilation, int groups) -> Tensor");
  m.def("conv3d_fprop(Tensor input, Tensor weight, Tensor? bias, int[] stride, int[] padding, int[] dilation, int groups) -> Tensor");
  m.def("conv3d_dgrad(Tensor grad_output, Tensor weight, int[] input_size, int[] stride, int[] padding, int[] dilation, int groups) -> Tensor");
  m.def("conv3d_wgrad(Tensor grad_output, Tensor input, int[] kernel_size, int[] stride, int[] padding, int[] dilation, int groups) -> Tensor");
}
