...
struct ConvGemm {
  ...
  using CollectiveEpilogue = cutlass::epilogue::collective::CollectiveEpilogue<
      EpilogueDispatchPolicy, TileShape, void, ElementAcc,
  ...
  static at::Tensor run(sycl::queue& queue, const at::Tensor& a, const at::Tensor& b) {
    ...
    typename Gemm::Arguments arguments{
        ...
        {M, N, K, L},
        {static_cast<const ElementInputA*>(a.data_ptr()), stride_A,
         static_cast<const ElementInputB*>(b.data_ptr()), stride_B},
    ...
    TORCH_CHECK(can_impl == cutlass::Status::kSuccess,
        "sycltla conv GEMM: invalid size M=", M, " K=", K, " N=", N);

    auto init_status = gemm_op.initialize(arguments, workspace_ptr);
    TORCH_CHECK(init_status == cutlass::Status::kSuccess, "sycltla conv GEMM init failed");

    auto run_status = gemm_op.run(&queue);
    TORCH_CHECK(run_status == cutlass::Status::kSuccess, "sycltla conv GEMM run failed");
...
inline at::Tensor run_conv_gemm(sycl::queue& queue, const at::Tensor& a, const at::Tensor& b) {
