    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        ...
        if (x_low >= 0 && x_high >= 0 && y_low >= 0 && y_high >= 0) {
          atomicAdd(
              (sycl_global_ptr<T>)(grad_input + offset + y_low * width + x_low),
              static_cast<T>(g1));
          atomicAdd(
              (sycl_global_ptr<T>)(grad_input + offset + y_low * width +
                                   x_high),
              static_cast<T>(g2));
          atomicAdd(
              (sycl_global_ptr<T>)(grad_input + offset + y_high * width +
                                   x_low),
              static_cast<T>(g3));
          atomicAdd(
              (sycl_global_ptr<T>)(grad_input + offset + y_high * width +
...
std::tuple<at::Tensor, at::Tensor> ps_roi_align_kernel(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "ps_roi_align_forward_kernel_xpu", [&] {
        sycl_kernel_submit<ps_roi_align_forward_kernel_impl<scalar_t>>(
...
Tensor ps_roi_align_backward_kernel(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad.scalar_type(), "ps_roi_align_backward_kernel_xpu", [&] {
        sycl_kernel_submit<ps_roi_align_backward_kernel_impl<scalar_t>>(
            global_range * local_range,
            local_range,
            at::xpu::getCurrentSYCLQueue(),
            0,
