...
struct KaiserWindowFunctor {
  scalar_t operator()(scalar_t a) const {
    ...
    opmath_t sqrt_term = std::max(opmath_t(0), opmath_t(1) - normalized_position * normalized_position);
    return calc_i0(static_cast<opmath_t>(beta_) * std::sqrt(sqrt_term)) * inv_i0_beta_;
  }
