...
void jagged_dense_elementwise_jagged_output_kernel_(
  ...
  const int offset_stride = item.get_group_range(1) * item.get_local_range(0);
...
#pragma unroll
    ...
    if (!truncated) {
      ...
      int iidx;
      for (iidx = item.get_local_id(1); iidx * 2 + 1 < inner_dense_size;
           iidx += item.get_local_range(1)) {
...
std::tuple<int64_t, int64_t, int64_t, StackArray<int64_t>>
...
void jagged_dense_elementwise_dense_output_kernel_(
  ...
  const int outer_begin =
      item.get_group(1) * item.get_local_range(0) + item.get_local_id(0);
  const int outer_stride = item.get_group_range(1) * item.get_local_range(0);
...
void jagged_dense_dense_elementwise_jagged_output_opt_search_kernel_(
  ...
  for (auto i = item.get_local_id(1); i < B + 1; i += item.get_local_range(1)) {
    offsets_sh[i] = offsets[i];
  }
  group_barrier(item.get_group());
  auto row = item.get_local_id(1) + item.get_group(1) * item.get_local_range(1);
  if (row >= nnz)
    return;
  int first = 1;
  int count = B - 1;
...
void jagged_dense_dense_elementwise_jagged_output_kernel_(
  ...
  const auto offset_begin =
      item.get_group(1) * item.get_local_range(0) + item.get_local_id(0);
  const auto offset_stride = item.get_group_range(1) * item.get_local_range(0);
  for (int offset = offset_begin; offset < nnz; offset += offset_stride) {
    int offset_temp = offset;
    int jidx = 0;
    bool truncated = false;
