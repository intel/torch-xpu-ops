...
void lstm_cell_forward_kernel_impl(
  ...
  bool has_bias1 = bias1.data != nullptr;
  bool has_bias2 = bias2.data != nullptr;
...
template <
    ...
    int indexing_kind>
SYCL_EXT_ONEAPI_FUNCTION_PROPERTY((syclexp::nd_range_kernel<1>))
void lstm_cell_backward_kernel_impl(
...
template <
    ...
    int indexing_kind>
SYCL_EXT_ONEAPI_FUNCTION_PROPERTY((syclexp::nd_range_kernel<1>))
void gru_cell_forward_kernel_impl(
    TensorInfo<scalar_t, index_type> Input,
    TensorInfo<scalar_t, index_type> Hidden,
    TensorInfo<scalar_t, index_type> Bias1,
    TensorInfo<scalar_t, index_type> Bias2,
...
template <
    ...
    int indexing_kind>
SYCL_EXT_ONEAPI_FUNCTION_PROPERTY((syclexp::nd_range_kernel<1>))
void gru_cell_backward_kernel_impl(
...
void lstm_forward_impl(
  ...
  if (allContiguous(
    ...
    collapseDims(
        ...
        workspaceI);
    constexpr auto kptr =
        lstm_cell_forward_kernel_impl<scalar_t, accscalar_t, index_type, 1>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    ...
    constexpr auto kptr =
        lstm_cell_forward_kernel_impl<scalar_t, accscalar_t, index_type, 2>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    auto config = rnn_get_launch_config(max_wg_size, numel);
    auto nwg = std::get<0>(config);
    auto local_range = std::get<1>(config);
...
void lstm_backward_impl(
  ...
  if (allContiguous(
    ...
    constexpr auto kptr =
        lstm_cell_backward_kernel_impl<scalar_t, accscalar_t, index_type, 1>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    auto config = rnn_get_launch_config(max_wg_size, numel);
    auto nwg = std::get<0>(config);
    auto local_range = std::get<1>(config);
    ...
    constexpr auto kptr =
        lstm_cell_backward_kernel_impl<scalar_t, accscalar_t, index_type, 2>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    auto config = rnn_get_launch_config(max_wg_size, numel);
    auto nwg = std::get<0>(config);
    auto local_range = std::get<1>(config);
...
void gru_forward_impl(
  ...
  if (allContiguous(
    ...
    collapseDims(
        ...
        workspaceI);
    constexpr auto kptr =
        gru_cell_forward_kernel_impl<scalar_t, accscalar_t, index_type, 1>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    auto config = rnn_get_launch_config(max_wg_size, numel);
    auto nwg = std::get<0>(config);
    auto local_range = std::get<1>(config);
    ...
    constexpr auto kptr =
        gru_cell_forward_kernel_impl<scalar_t, accscalar_t, index_type, 2>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    auto config = rnn_get_launch_config(max_wg_size, numel);
    auto nwg = std::get<0>(config);
    auto local_range = std::get<1>(config);
...
void gru_backward_impl(
  ...
  if (allContiguous(
    ...
    constexpr auto kptr =
        gru_cell_backward_kernel_impl<scalar_t, accscalar_t, index_type, 1>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    auto config = rnn_get_launch_config(max_wg_size, numel);
    auto nwg = std::get<0>(config);
    auto local_range = std::get<1>(config);
    ...
    constexpr auto kptr =
        gru_cell_backward_kernel_impl<scalar_t, accscalar_t, index_type, 2>;
    auto max_wg_size = at::xpu::getKernelMaxWorkGroupSize<kptr>();
    auto config = rnn_get_launch_config(max_wg_size, numel);
    auto nwg = std::get<0>(config);
    auto local_range = std::get<1>(config);
