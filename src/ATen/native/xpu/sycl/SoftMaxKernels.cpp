        ...
        if (LogSoftMax) {
          if constexpr (is_same_dtype) {
            reg_out[i][j] = static_cast<outscalar_t>(
                reg_gradout[i][j] - sycl::exp(reg_out[i][j]) * sum_value);
          } else {
...
#pragma unroll(vec_size)
        for (int j = 0; j < vec_size; ++j) {
          ...
          if (linear_idx >= 0 && linear_idx < dim_size_) {
            ...
            if (LogSoftMax) {
              gradInput_[offset] =
                  gradOutput_[offset] - sycl::exp(output_[offset]) * sum_value;
            } else {
              gradInput_[offset] =
...
#pragma unroll(vec_size)
        for (int j = 0; j < vec_size; ++j) {
          if constexpr (is_same_dtype) {
            if (LogSoftMax) {
              out_val[j] = grad_val[j] - sycl::exp(out_val[j]) * sum_value;
            } else {
            ...
            if (LogSoftMax) {
              gradInput_[offset] =
                  grad_val[j] - sycl::exp(out_val[j]) * sum_value;
            } else {
          ...
          if constexpr (is_same_dtype) {
            if (LogSoftMax) {
              out_val[j] = static_cast<outscalar_t>(
                  gradout_val[j] - sycl::exp(out_val[j]) * sum_value[j]);
            } else {
              out_val[j] = static_cast<outscalar_t>(
