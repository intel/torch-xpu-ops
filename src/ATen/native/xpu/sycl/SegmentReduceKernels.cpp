  ...
  void operator()(sycl::nd_item<1> item) const {
    ...
    for (index_t j = offset_start; j < offset_end; ++j) {
      ...
      if (reduction_ == native::ReductionType::MAX) {
        initial_value =
            sycl::isnan(static_cast<at::opmath_type<scalar_t>>(data)) ? data : std::max<scalar_t>(initial_value, data);
      } else if (
        ...
        initial_value =
            sycl::isnan(static_cast<at::opmath_type<scalar_t>>(data)) ? data : std::min<scalar_t>(initial_value, data);
    ...
    if (lengths_data_[lengths_idx] == 0 && !is_initial_set_ &&
      ...
      initial_value = static_cast<scalar_t>(NAN);
        ...
        reduction_ == native::ReductionType::MEAN &&
        lengths_data_[lengths_idx] > 0 && !sycl::isnan(static_cast<at::opmath_type<scalar_t>>(initial_value))) {
  ...
  void operator()(sycl::nd_item<1> item) const {
    ...
    if (reduction_ == native::ReductionType::MAX ||
      ...
      for (int64_t j = offset_start; j < offset_end; ++j) {
        int64_t data_index = outer_idx * data_stride_axis_ * data_size_axis_ +
            j * data_stride_axis_ + lane_id;
        if (sycl::isnan(static_cast<at::opmath_type<scalar_t>>(values_data_[data_index])) ||
