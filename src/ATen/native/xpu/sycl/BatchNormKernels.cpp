...
std::tuple<Tensor&, Tensor&, Tensor&> batch_norm_kernel(
  ...
  if (train) {
    ...
    // CPU/CUDA behavior. Use local tensors for the elementwise computation.
    TORCH_CHECK(has_running_mean);
    auto options =
        self.options().dtype(at::toAccumulateType(self.scalar_type(), kXPU));
    auto mean_ = at::empty_like(*running_mean_opt, options);
    auto invstd_ = at::empty_like(*running_var_opt, options);
    mean_.copy_(*running_mean_opt, /*non_blocking=*/true);
    batch_norm_calc_invstd(invstd_, running_var_opt.value(), epsilon);
    batch_norm_elementwise(output, self, weight_opt, bias_opt, mean_, invstd_);
