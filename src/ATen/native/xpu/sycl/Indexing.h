...
int64_t inline indexing_logical_off(
  ...
  int64_t glb_batch_group_glb_off =
      glb_batch_group * cfg_.indexing_dimension_size_ + glb_batch_group_loc_off;
...
void index_kernel(IdxConfig cfg_) {
  ...
  init_global_batch_info<IdxConfig, TrivialOffCal, KnownProblemInner>(
      id, idx_logical_off, glb_batch_group, glb_batch_group_loc_off, cfg_);

  glb_indexing_logical_off =
      indexing_logical_off<IdxConfig, TrivialOffCal, KnownProblemInner>(
          id, glb_batch_group, glb_batch_group_loc_off, cfg_);

  if (cfg_.sinfo_.data != nullptr && cfg_.dinfo_.data != nullptr) {
    glb_fixing_logical_off =
        fixing_logical_off<IdxConfig, TrivialOffCal, KnownProblemInner>(
            id, glb_batch_group, idx_logical_off, cfg_);
  }
...
void small_index_kernel(
  ...
  if (group_num_tail_ && group_id >= group_num_) {
    group_linear_id =
        group_num_ * group_numel_ + (group_id - group_num_) * group_numel_tail_;
...
void small_index_kernel(
  ...
  auto wgroup_size =
      syclMaxWorkGroupSize<small_index_kernel<func_t, index_buf_type>>();
  ...
  sycl_kernel_submit<small_index_kernel<func_t, index_buf_type>, 1>(
...
void index_kernel_impl(
  ...
  sycl_kernel_submit<
      index_functor_kernel<func_t, index_buf_type, offset_calc_type>,
      1>(
      sycl::range<1>(numel),
      sycl::range<1>(1),
      queue,
...
void launch_index_put_deterministic_kernel(
  ...
  BatchKernelConfig cfg = BatchKernelConfig::make_config<
      index_put_deterministic_kernel<scalar_t, accscalar_t>>(
  ...
  sycl_kernel_submit<index_put_deterministic_kernel<scalar_t, accscalar_t>>(
      cfg.global_size(),
      cfg.group_size(),
      getCurrentSYCLQueue(),
      0,
...
SYCL_EXT_ONEAPI_FUNCTION_PROPERTY((syclexp::nd_range_kernel<1>))
void index_elemnent_wise_kernel(const int64_t N_, const func_t f_) {
...
static void launch_index_group_stride_kernel(const int64_t N, const func_t& f) {
  ...
  sycl_kernel_submit<index_elemnent_wise_kernel<vt, func_t>, 1>(
...
template <int vt, typename func_t>
SYCL_EXT_ONEAPI_FUNCTION_PROPERTY((syclexp::nd_range_kernel<1>))
void take_put_kernel(const int64_t N_, const func_t f_) {
  auto item = syclext::this_work_item::get_nd_item<1>();
