...
class BatchKernelConfig {
 ...
 public:
  ...
  static Policy suggest_policy(
    ...
    // Using device max work group size to deduce range configuration
    // approximately.
...
private:
 ...
 void build() {
   ...
   wg_range_y_ = std::min<size_t>(
       wg_size / wg_range_x_, c10::llvm::PowerOf2Ceil((uint64_t)range_bound_y));
   ...
   if ((uint8_t)policy_ & (uint8_t)Policy::pAdaptive) {
     ...
     if (problem_along_x_) {
       ...
       glb_range_y_ = size_t((problem_batch_ + wg_range_y_ - 1) / wg_range_y_) *
           wg_range_y_;
     } else {
       glb_range_x_ = size_t((problem_batch_ + wg_range_x_ - 1) / wg_range_x_) *
           wg_range_x_;
