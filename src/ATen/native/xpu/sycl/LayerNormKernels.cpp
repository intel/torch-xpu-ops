...
// ===================================================================
// RMS Norm kernel implementation (ported from ipex/src/rms_norm.cpp)
// ===================================================================
...
// Large-N / arbitrary-N RMSNorm kernel: one workgroup per row.
...
void rms_norm_kernel_impl(
  ...
  // Dispatch:
  //   N <= 16384: IPEX-ported kernel (arbitrary-N via aligned fast path +
  //               scalar masked fallback).
  //   N >  16384: Large-N kernel (one workgroup per row).
  ...
  constexpr int kAlignBytes = rms_norm_impl_detail::granularity; // 16
...
// ===================================================================
// End of RMS Norm kernel implementation
// ===================================================================
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        ...
        rms_norm_kernel_impl<scalar_t, acc_t>(
            X, gamma, M, N, static_cast<acc_t>(eps), Y, rstd);
      });
}
