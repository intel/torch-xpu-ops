...
struct GluFunctor(
  ...
  scalar_t operator()(scalar_t a_, scalar_t b_) {
    const opmath_t a = a_;
    const opmath_t b = b_;
...
struct GluBackwardKernelFunctor() {
  void operator()(sycl::nd_item item) {
    using opmath_t = at::opmath_type<scalar_t>;
    ...
    const opmath_t b = *byte_offset_(I + offsets[1], I_byte_offset_);
  ...
  GluBackwardKernelFunctor(
    ...
    int64_t I_byte_offset) : numel_(numel), gI_(gI), I_(I), gO_(gO), offset_calculator_(offset_calculator), gI_byte_offset_(gI_byte_offset), I_byte_offset_(I_byte_offset) {}
...
void launch_glu_backward_kernel(
  ...
  const int64_t global_size = (N + local_size - 1) / local_size;
...
void glu_backward_kernel(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      kHalf, kBFloat16, iter.common_dtype(), "glu_backward_xpu", [&] {
        ...
        launch_glu_backward_kernel<typename scalar_t, typename OffsetCalc>(
            N,
            gI,
            I,
            gO,
