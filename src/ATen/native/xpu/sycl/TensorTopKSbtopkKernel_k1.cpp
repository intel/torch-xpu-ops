...
/*
 * Subgroup top-k kernel -- K=1 instantiations.
 *
...
static void sbtopk_k1_typed(
    ...
    bool largest) {
  if (largest) {
    sbtopk_launch_vec_dispatch<scalar_t, 1, true, IndexT>(
        input, topK, indices, numSlices, sliceSize, k);
  } else {
    sbtopk_launch_vec_dispatch<scalar_t, 1, false, IndexT>(
        input, topK, indices, numSlices, sliceSize, k);
  }
}
  ...
  AT_DISPATCH_ALL_TYPES_AND2(
      ...
      [&]() {
        ...
        if (nsegments <=
          ...
          sbtopk_k1_typed<scalar_t, int64_t>(
