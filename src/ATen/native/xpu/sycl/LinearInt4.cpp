  ...
  AT_DISPATCH_REDUCED_FLOATING_TYPES(
      A.scalar_type(), "linear_int4_kernel", [&]() {
        ...
        const scalar_sycl_t* input_data =
          reinterpret_cast<const scalar_sycl_t*>(A.const_data_ptr<scalar_t>());
        const uint8_t* weight_data =
          reinterpret_cast<const uint8_t*>(B.const_data_ptr()); // int4x2 or int4x8

        scalar_sycl_t* output_data =
            reinterpret_cast<scalar_sycl_t*>(C.data_ptr<scalar_t>());
        const scalar_sycl_t* scale_zeros_data = reinterpret_cast<const scalar_sycl_t*>(
