...
void upsample_bicubic2d_out_fn(
...
static void launch_upsample_bicubic2d_out_kernel(
  ...
  int64_t wg_size =
      syclMaxWorkGroupSize<upsample_bicubic2d_out_fn<scalar_t, accscalar_t>>();
  int64_t num_wg = at::ceil_div(onum, wg_size);
  auto queue = getCurrentSYCLQueue();

  sycl_kernel_submit<upsample_bicubic2d_out_fn<scalar_t, accscalar_t>>(
      num_wg * wg_size,
      wg_size,
      queue,
      0,
...
SYCL_EXT_ONEAPI_FUNCTION_PROPERTY(
    (sycl::ext::oneapi::experimental::nd_range_kernel<1>))
void upsample_bicubic2d_backward_out_fn(
    int num_elements,
    accscalar_t height_scale,
    accscalar_t width_scale,
    bool align_corners,
...
static void launch_upsample_bicubic2d_backward_out_kernel(
  ...
  int64_t wg_size = syclMaxWorkGroupSize<
      upsample_bicubic2d_backward_out_fn<scalar_t, accscalar_t>>();
  int64_t num_wg = at::ceil_div(num_elements, wg_size);
  auto queue = getCurrentSYCLQueue();

  sycl_kernel_submit<upsample_bicubic2d_backward_out_fn<scalar_t, accscalar_t>>(
      num_wg * wg_size,
      wg_size,
      queue,
      0,
