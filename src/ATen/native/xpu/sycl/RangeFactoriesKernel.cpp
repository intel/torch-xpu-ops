...
Tensor& logspace_kernel(
  ...
  if (steps == 0) {
    ...
    // EXPERIMENT: Drop complex dispatch so complex<float> won't be instantiated in this TU.
    // Without complex<float> powf kernel, its ContractionOff no longer leaks onto the shared powf path,
    // so float logspace should regress to 0x41200001 (1 ULP high) also on Linux.
