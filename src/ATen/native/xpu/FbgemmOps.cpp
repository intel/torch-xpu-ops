...
Tensor dense_to_jagged_forward_xpu(
  ...
  xpu::dense_to_jagged_forward_xpu_kernel(values, offsets, dense, output);
...
Tensor jagged_to_padded_dense_forward_xpu(
  ...
  xpu::jagged_to_padded_dense_forward_xpu_kernel(
...
class JaggedDenseAddJaggedOutputOp
 ...
 public:
  static torch::autograd::variable_list forward(
    ...
    XPU_DEVICE_GUARD(dense);
    xpu::jagged_dense_elementwise_add_jagged_output_fwd_xpu_kn(
...
Tensor reorder_batched_ad_lengths_xpu(
  ...
  xpu::reorder_batched_ad_lengths_xpu_kernel(
      cat_ad_lengths,
      batch_offsets,
...
Tensor reorder_batched_ad_indices_xpu(
  ...
  xpu::reorder_batched_ad_indices_xpu_kernel(
      cat_ad_offsets,
      cat_ad_indices,
...
permute_2D_sparse_data_xpu(
  ...
  if (weights.has_value()) {
    ...
    xpu::permute_2D_data_kernel_xpu(
...
TORCH_LIBRARY_IMPL(fbgemm, XPU, m) {
  m.impl(
      ...
      &at::native::asynchronous_complete_cumsum_xpu);
  m.impl("dense_to_jagged", &at::native::dense_to_jagged);
  m.impl("dense_to_jagged_forward", &at::native::dense_to_jagged_forward_xpu);
  m.impl("jagged_to_padded_dense", &at::native::jagged_to_padded_dense);
  m.impl(
      "jagged_to_padded_dense_forward",
      &at::native::jagged_to_padded_dense_forward_xpu);
  m.impl(
      "jagged_dense_elementwise_add_jagged_output",
      &at::native::jagged_dense_elementwise_add_jagged_output_xpu);
  m.impl(
      "reorder_batched_ad_lengths",
      &at::native::reorder_batched_ad_lengths_xpu);
  m.impl(
