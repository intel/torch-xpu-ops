...
std::tuple<Tensor, Tensor> _fused_rms_norm_backward_xpu(
  ...
  if (M > 0 && N > 0) {
    // rstd is stored with stat_shape (e.g. [B, 1, 1]); flatten to [M] for
    // kernel
    Tensor rstd_flattened = rstd.contiguous().view({M});
    xpu::rms_norm_backward_kernel(
        dY, *X, rstd_flattened, *gamma, M, N, &dX, &dgamma);
  }
