...
Tensor bmm_nested_xpu(const Tensor& self, const Tensor& mat2) {
  ...
  auto self_ptr = self.is_nested() ? get_nested_tensor_impl(self)
                                   : self.unsafeGetTensorImpl();
  auto mat2_ptr = mat2.is_nested() ? get_nested_tensor_impl(mat2)
  ...
  for (int64_t i = 0; i < ntensors; i++) {
    const IntArrayRef &self_shape = get_size_for_index(self, i),
                      &mat2_shape = get_size_for_index(mat2, i);
  ...
  // Use mm_out for each nested matrix multiplication
