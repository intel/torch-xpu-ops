 ...
 public:
  ISHMEMPeerAllocInfo(
    ...
    if (cache_it == rank_to_global_rank_cache.end()) {
      ...
      if (rank_ == 0) {
        LOG(INFO) << "[rank " << rank_ << ']'
                  << " rank_to_global_rank exchanged"
                  << ", group_name: " << group_name
                  << ", exchanged_n_times: " << exchanged_n_times;
      }
    }
    rank_to_global_rank_ = rank_to_global_rank_cache[group_name];
    ...
    signal_pad_raw_ptr_ = ishmem_malloc(get_signal_pad_size());
    TORCH_CHECK(signal_pad_raw_ptr_ != nullptr, "ishmem_malloc failed");

    auto& queue = at::xpu::getCurrentSYCLQueue();
    queue.memset(signal_pad_raw_ptr_, 0, get_signal_pad_size()).wait();
